...
//   // Fetch open positions when selected account changes
//   useEffect(() => {
...
//       setError(null)
//       try {
//         console.log(`Fetching positions for account: ${selectedAccount}`)
//         // ====== CHANGED: Now real API call ======
//         const response = await getAPI(`${endpoints.getOpenPositions}?Trade_Account=${selectedAccount}`)
//         console.log('Open positions response:', response)
...
//             console.log('No open positions found for this account')
//           }
//         } else {
//           setOpenPositions([])
...
//   // Calculate total P&L
//   const totalPL = openPositions.reduce((sum, position) => sum + (position.floating_profit_loss || 0), 0).toFixed(8)
//   const totalPLUSD = openPositions.reduce((sum, position) => sum + (position.floating_profit_loss_usd || 0), 0).toFixed(2)
...
//     tableHeader: {
//       display: 'grid',
//       gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr 1.5fr 1fr',
//       padding: '16px',
...
//     return (
//       <div style={styles.container}>
//         <div style={{ color: '#ef4444', padding: '16px', backgroundColor: 'rgba(239, 68, 68, 0.1)', borderRadius: '4px' }}>{error}</div>
//       </div>
//     )
//   }
...
//         <h1 style={styles.title}>Call Options</h1>
//         <div style={styles.positionCount}>{openPositions.length} Positions</div>
//       </div>

//       <div style={styles.plSection}>
//         <div style={styles.plLabel}>Total P&L:</div>
//         <div style={styles.plValue}>
//           ↑ {totalPL} <span style={styles.usdValue}>({totalPLUSD} USD)</span>
//         </div>
//       </div>
...
//           <div style={styles.tableHeaderCell}>Index Price</div>
...
//           <div style={styles.emptyMessage}>No open positions found</div>
//         ) : (
...
//                       {/* <div style={styles.currencyIcon}>
//                         <Image src={`/crypto-${position.currency.toLowerCase()}.png`} alt={position.currency} width={20} height={20} />
//                       </div> */}
...
//                     {position.size}
//                   </div>
//                   <div style={styles.tableCell}>{position.index_price.toLocaleString()}</div>
//                   <div style={styles.tableCell}>
//                     <div style={styles.plCell}>
//                       <div style={{ ...styles.plCrypto, ...(isProfitable ? styles.positive : styles.negative) }}>
//                         {position.floating_profit_loss.toFixed(8)}
//                       </div>
//                       <div style={{ ...styles.plFiat, ...(isProfitable ? styles.positive : styles.negative) }}>
//                         ${position.floating_profit_loss_usd.toFixed(2)}
//                       </div>
//                     </div>
//                   </div>
//                   <div style={styles.tableCell}>
//                     <div style={{ ...styles.pill, ...styles.openPill }}>{position.status}</div>
//                   </div>
//                 </div>
//               )
//             })}
...
const OpenPositions = ({ params }) => {
  ...
  useEffect(() => {
    const fetchOpenPositions = async () => {
      ...
      setError(null)
      try {
        console.log(`Fetching open positions for account: ${selectedAccount}`)
  ...
  const totalPL = openPositions.reduce((sum, position) => sum + (position.total_profit_loss || 0), 0).toFixed(8)
  const totalPLUSD = openPositions.reduce((sum, position) => sum + (position.total_profit_loss_usd || 0), 0).toFixed(2)
  const isPLPositive = Number.parseFloat(totalPL) >= 0
              ...
              <div className="clearfix">
                <div style={styles.header}>
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <h4 style={styles.title}>Open Positions</h4>
                    <div style={styles.positionCount}>{openPositions.length} Positions</div>
                  </div>
                </div>
              </div>
